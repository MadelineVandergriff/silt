...
pub unsafe fn copy_entire_buffer(loader: &Loader, pool: &CommandPool, src: &Buffer, dst: &Buffer) -> Result<()> {
...
pub unsafe fn copy_buffer_to_image(loader: &Loader, pool: &CommandPool, src: &Buffer, dst: &Image, region: vk::BufferImageCopy) -> Result<()> {
    ...
    if !Volume3D::from(dst.size).contains(&region_volume) {
        return Err(anyhow!("Region extents past image bounds: [{:?}] [{:?}]", region, dst));
    }
...
pub unsafe fn copy_buffer_to_whole_image(loader: &Loader, pool: &CommandPool, src: &Buffer, dst: &Image) -> Result<()> {
...
impl<'a, T: Copy> Drop for MemoryMapping<'a, T> {
    fn drop(&mut self) { unsafe {
        let allocator = &self.loader.allocator;
        let device = &self.loader.device;

        if allocator.get_mapped_ptr(self.allocation).is_err() {
            device.unmap_memory(allocator.get_memory(self.allocation).unwrap());
        }
    }}
}
...
pub unsafe fn map_buffer<'a, T: Copy>(loader: &'a Loader, buffer: &Buffer) -> MemoryMapping<'a, T> {
    ...
    let align = Align::new(ptr, std::mem::align_of::<T>() as u64, buffer.size);
    MemoryMapping::new(loader, buffer.allocation, align)
}

pub fn upload_to_gpu<T: Copy>(loader: &Loader, pool: &CommandPool, data: &[T], usage: vk::BufferUsageFlags, name: Option<&'static str>) -> Result<Buffer> {
